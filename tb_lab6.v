...
initial begin
  ...
  */
  ...
  fork : wait_or_timeout
  begin
    repeat(1000) begin // <--- ADD 'begin' HERE
      @(posedge clk);
      
      // INSERT DEBUG CODE HERE
      //$display("Time: %0t | Clk Count: %0d | State: %0d | row: %0d, col: %0d, k: %0d | AddrA: %0d, AddrB: %0d | MAC Out: %0d | MAC2 OUT: %0d | MAC3 OUT: %0d | MAC4 OUT: %0d",
      //          $time, clock_count, DUT.state, DUT.row, DUT.col, DUT.k, DUT.addr_A, DUT.addr_B, DUT.mac_out, DUT.mac_out2, DUT.mac_out3, DUT.mac_out4);
      //$display("Time: %0t | k: %0d | Buffer 2 Value: %d | Buffer 3 value: %d | Buffer 4 value: %d", $time, DUT.k, DUT.mac_out2_buffer, DUT.mac_out3_buffer, DUT.mac_out4_buffer);
    end // <--- ADD 'end' HERE
    
    disable wait_or_timeout; // Ensure we kill the other fork branch if we hit 1000
  end
  join
  // End Timeout Routing
  //-------------------------
